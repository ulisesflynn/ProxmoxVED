...
# ---- Seed credentials into the freshly built container ----
# Only runs if the install script actually created /opt/idrac-certs. Guarding on
# this keeps a failed/404'd install from hard-failing here and auto-removing the CT.
if [[ "$CONFIGURE_CREDS" == "yes" ]]; then
  if pct exec "$CTID" -- test -d /opt/idrac-certs 2>/dev/null; then
    ...
    msg_error "/opt/idrac-certs not found in CT ${CTID} - install script likely did not run (check for a 404 above). Skipping credential seeding; container left intact for inspection."
  fi
fi
